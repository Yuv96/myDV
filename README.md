...
每次切入视频后，资料卡从视频开始出画面起显示 3 秒；循环播放、暂停恢复和页面返回不重新计时。菜单键打开右侧互动菜单时临时召回资料卡，菜单关闭立即隐藏；切到其他视频后再切回来，会重新显示一次。旧版“常驻/6 秒”偏好不再控制此行为。资料卡不再显示清晰度；清晰度选项保留在设置中。时间与通知数显示在右上角。
