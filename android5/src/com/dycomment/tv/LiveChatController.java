...
public final class LiveChatController {
    ...
    String room = "", cursor = "", session = "", uid = "";
    volatile int epoch;
    int empty;
    volatile boolean running;
    boolean closed;

    LiveChatController(Activity a) {
        activity = a;
    }
    ...
    static LiveChatController get(Activity a) {
        ...
        LiveChatController c = new LiveChatController(a);
        decor.setTag(TAG, c);
        return c;
    }

    public static void start(Activity a, String id) {
        LiveChatController c = get(a);
        stop(a);
        if (id == null || !id.matches("[0-9]+") || c.closed) return;
        c.room = id;
        c.session = SocialApi.cookie();
        c.running = true;
        c.poll();
    }
    ...
    void poll() {
        ...
        final String roomId = room, cookie = session, previous = cursor, user = uid;
        work.execute(
                () -> {
                    if (!running || token != epoch) return;
                    String next = previous, account = user;
                    List<Chat> chats = new ArrayList<>();
                    boolean ok = false;
                    try {
                        if (account.isEmpty()) {
                            org.json.JSONObject self =
                                    SocialApi.request(
                                                    ...
                                                    cookie)
                                            .optJSONObject("user");
                            if (self == null) throw new Exception();
                            account = self.optString("uid", "");
                        }
                        String query =
                                SocialApi.encode(
                                        SocialApi.params(
                        ...
                        try {
                            ...
                            try (InputStream in = c.getInputStream()) {
                                response = new Wire(QuickShareApi.read(in));
                            }
                            if (response.first(2) == null) throw new Exception();
                            next = response.text(2);
                    ...
                    final List<Chat> result = chats;
                    final String nextCursor = next, knownUser = account;
                    ...
                    main.post(
                            () -> {
                                ...
                                cursor = nextCursor;
                                uid = knownUser;
                                if (!success || result.isEmpty()) empty++;
                                else empty = 0;
