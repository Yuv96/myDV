...
final class QrSession implements AutoCloseable {
    ...
    private byte[] request(String address, Map<String, String> body, int redirects)
            throws Exception {
        if (closed) throw new IOException("登录已取消");
        URL url = new URL(address);
        if (!allowed(url) || redirects > 5) throw new IOException("登录跳转地址无法验证");
        ...
        try {
            if (body != null) {
                ...
                byte[] bytes = SocialApi.encode(body).getBytes("UTF-8");
            ...
            if (status >= 300 && status < 400) {
                ...
                c.disconnect();
                return request(next.toString(), null, redirects + 1);
            }
            ...
            try (InputStream in = c.getInputStream();
                ...
                while ((n = in.read(buffer)) != -1) {
                    ...
                    if (out.size() + n > 1024 * 1024
                        ...
                        throw new IOException("登录响应超出限制");
    ...
    private JSONObject passport(String path, Map<String, String> body) throws Exception {
        ...
        JSONObject result =
                new JSONObject(
                        new String(
                                request(ORIGIN + path + "?" + SocialApi.encode(query), body, 0),
                                "UTF-8"));
    ...
    String create() throws Exception {
        ...
        String image = data.optString("qrcode");
        if (token.isEmpty() || image.isEmpty() || image.length() > 700000)
            throw new IOException("登录服务没有返回二维码");
        return image;
    }
    ...
    String finish(JSONObject data) throws Exception {
        ...
        for (HttpCookie c : jar.getCookieStore().getCookies()) {
            ...
            if (!c.hasExpired()
                    && domain != null
                    && (domain.equals("douyin.com")
                            || domain.equals(".douyin.com")
                            || domain.equals("www.douyin.com"))) {
        ...
        if (self == null || self.optString("uid").isEmpty())
            throw new IOException("新账号验证未通过；现有账号已保留");
        return session;
    }
