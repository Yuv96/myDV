...
final class CredentialStore {
    static String value(String cookie, String name) {
        ...
        for (String part : cookie.split(";")) {
            int at = part.indexOf('=');
            if (at > 0 && name.equals(part.substring(0, at).trim()))
    ...
    static void save(Context context, String cookie) throws Exception {
        ...
        // Commit the new account only after the caller has verified it; never clear a working one
        // on failure.
        Class.forName("com.dycomment.tv.DouyinApi")
                .getMethod("setCookie", String.class, Context.class)
                .invoke(null, cookie, context);
        android.content.SharedPreferences.Editor edit =
                context.getSharedPreferences("dy_config", 0).edit();
        edit.remove("a_bogus").remove("ms_token").remove("ms_token_time");
        String token = value(cookie, "msToken");
        if (!token.isEmpty())
            edit.putString("ms_token", token).putLong("ms_token_time", System.currentTimeMillis());
        edit.apply();
