...
上游公开仓库不包含 Android 工程。本目录以 SHA-256 固定的官方 Lite 1.1.9 APK 为输入，通过 Apktool + javac + D8 重建。a5.1 替换 `PlayerView` 并添加 `NextVideoCache`；a5.2 保留这两者，增加纯文字菜单、遥控器互动面板、关注直播及未读通知提示。a5.3 修复慢网络下的播放器释放阻塞、快速往返的旧请求覆盖及底栏提前切换，详见 [切换修复说明](SWITCHING-NOTES.md)。a5.4 修复从作者主页返回时的画面重绑，加入直播预览图、竖屏比例修正和整体资料卡，详见 [本轮说明](UI-RETURN-NOTES.md)。没有降低声明版本来掩盖新 API，最低系统保持 API 21。

a5.2 的功能范围、验证方法和未完成事项见 [INTERACTION-NOTES.md](INTERACTION-NOTES.md)。a5.4 新增好友视频卡片分享入口（发送仍需实际使用验证），未读角标仍是通知数，暂不包含私信。点赞、关注、收藏写操作仍需实际使用验证，开发验证没有代替用户执行这些账号操作。
