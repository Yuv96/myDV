...
应用：抖音抬头版 0.1.3。最低 API 21，目标 API 30，包名 `com.dycomment.tv.android5`，版本号 1008。保留既有签名才能覆盖此前的 Android5 社区版。
...
正式包输出到 `dist/Douyin-TV-0.1.3.apk`，不包含测试 Activity 或测试视频。`BUILD_WORK=/tmp/douyin-build` 可指定临时构建目录；依赖下载后校验固定 SHA-256。
...
CI 下载 SHA-256 固定的 a5.4、v0.1.1 和 v0.1.2 发布包，读取真实证书并比对固定指纹；新正式包必须与三者一致。API 21 模拟器另执行 a5.4 → v0.1.1 → v0.1.2 → v0.1.3 的 `adb install -r` 升级链，并确认应用私有数据标记保留。`SIGNING_CERTIFICATE.txt` 随正式包提供公开证书指纹。

仅主分支可以手动触发工作流的 `publish=true`。只有编译、完整回归、签名比对及真实升级安装全部通过，发布任务才会再次验证下载的产物并创建 v0.1.3。
...
`InteractionController` 负责遥控器路由和菜单分组，`ModernMenuHelper`、`UiTheme`、`CommentsPanel` 提供统一组件。`CredentialStore` 管理兼容存储，`QrLoginActivity` 使用可见的官方网页扫码，`OfficialLoginPolicy` 限制 HTTPS 导航与验证候选账号，`CredentialHealth` 从实际失败频率产生可恢复提示。`SocialApi` 与 `LegacyRequests` 管理新旧接口边界。
...
旧简化 HTTP 扫码轮询在当前网络返回 4031，同网络官方网页轮询成功。生产入口现改为官方网页；GitHub 的 API21 界面/安全回归不能代替真实电视上的网页兼容性及手机扫码确认。扫码后的评论等动态签名仍需独立联调。
